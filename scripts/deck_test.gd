...
@export var hand_start: Vector2 = Vector2.ZERO
@export var hand_spacing: float = 190.0
@export var hand_padding: float = 10.0

@onready var deck_spawn: Node2D = $DeckSpawnPoint
...
func _layout_hand() -> void:
	...
	if not _hand_cards.is_empty():
		var base_width := _get_card_width(_hand_cards[0])
		spacing = max(hand_spacing, base_width + hand_padding)
	for i in range(_hand_cards.size()):
		var card := _hand_cards[i]
		card.position = hand_start + Vector2(i * spacing, 0)
		card.z_index = i
...
func _get_card_width(card: Control) -> float:
	var shadow := card.get_node_or_null("Shadow") as Control
	if shadow != null:
		var size := shadow.size
		if size == Vector2.ZERO:
			size = shadow.custom_minimum_size
		if size.x > 0:
			return size.x * card.scale.x
	return hand_spacing
